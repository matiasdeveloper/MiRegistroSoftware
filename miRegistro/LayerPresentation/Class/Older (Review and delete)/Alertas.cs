...
namespace LayerPresentation.Clases
{
    public static class Alertas
    {
        ...
        private static void BuscarAlertaStock(int StockMenorQue)
        {
            ...
            {
                /*DataTable nuevaAlerta = UtilitiesCommon.layerBusiness.cn_alertas.buscarNuevasAlertas(StockMenorQue);
                if (!(nuevaAlerta.Rows.Count == 0))
                {
                    DateTime now = new DateTime();
                    now = DateTime.Now;
                    int id = UtilitiesCommon.layerBusiness.cn_alertas.insertarNuevaAlerta(UserLoginCache.Username);
                    foreach (DataRow fila in nuevaAlerta.Rows)
                    {
                        string valor = fila[0].ToString();
                        UtilitiesCommon.layerBusiness.cn_alertas.insertarDetallesAlerta(id, Convert.ToInt32(valor), "Alerta de stock");
                    }
